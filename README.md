...
- `ProblemDetails` models RFC 9457-compatible payloads, including valid partial
  documents and extension fields. `StandardProblemDetails` describes the fully
  populated payloads generated by this package; `ValidationProblemDetails` adds
  field-level validation errors.
- `StandardError` registers HTTP status/title pairs and returns
  `Result<never, StandardProblemDetails>` from `error()`. Its `types` getter
  returns an isolated read-only registry snapshot; use `add()` for intentional
  registration changes.
- `isProblemDetails`, `toErrorMessage`, `toProblemDetails`, and `problemResult`
  normalize unknown thrown values into `ProblemDetails`; `validateHttpResponse`
  preserves the actual status and title of a non-OK upstream response, retaining
  at most 8 KiB of its body detail by default and cancelling readable bodies when
  that limit is reached. Callers can customize the limit with
